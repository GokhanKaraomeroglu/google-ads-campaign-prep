...
const CampaignForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (demands.visualRequest) {
      ...
      visualRequestDocument += "(1). Quantity: <Specify>\n";
      visualRequestDocument += "(2). Messages: <Specify>\n";
      visualRequestDocument += "(3). Notes: <Specify>\n";
    
      products.forEach((product, index) => {
        visualRequestDocument += `b. Product Images (for Product-${index + 1})\n`;
        visualRequestDocument += `(1) Product-${index + 1}\n`;
        visualRequestDocument += `   (a) Quantity: <Specify>\n`;
        visualRequestDocument += `   (b) Messages: <Specify>\n`;
        visualRequestDocument += `   (c) Notes: <Specify>\n`;
      });
      
      visualRequestDocument += "c. Video Requests (2 Duration, 3 Ratio)\n";
      visualRequestDocument += "(1). Quantity: <Specify>\n";
      visualRequestDocument += "(2). Duration: (Seconds): <Specify>\n";
      visualRequestDocument += "(3). Messages: <Specify>\n";
      visualRequestDocument += "(4). Notes: <Specify>\n";
    }
  
    // Tüm verileri birleştirerek tek bir kullanıcı isteği oluşturun
    ...
    try {
      const apiResponse = await axios.post('https://api.openai.com/v1/chat/completions', {
        ...
        messages: [
          ...
          { role: 'user', content: JSON.stringify(combinedRequest) }, // Tek bir kullanıcı isteği
        ]
      }, {
        headers: {
